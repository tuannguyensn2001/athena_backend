...
describe MemberService::ApproveStudentPending do
  ...
  let!(:member_1) { FactoryBot.create(:member, user_id: student_1.id, workshop_id: workshop.id, role: :student, status: :pending) }
  let!(:member_2) { FactoryBot.create(:member, user_id: student_2.id, workshop_id: workshop.id, role: :student, status: :pending) }
  let!(:member_teacher) { FactoryBot.create(:member, user_id: teacher.id, workshop_id: workshop.id, role: :teacher, status: :active) }
