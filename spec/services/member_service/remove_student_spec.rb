...
describe MemberService::RemoveStudent do
  ...
  let!(:member_1) { FactoryBot.create(:member, user: teacher, workshop:, role: :teacher, status: :active) }
  let!(:member_2) { FactoryBot.create(:member, user: student, workshop:, role: :student, status: :active) }
  let(:params) do
    {
